...
namespace userRegistration
{
    class UserRegistration : Validator 
    {
        ...
        public string checkMatch(string exp, string stringToMatch) // interface method to match
        {
        ...
        public string validateName(string name) // interface method to validate name
        {
        ...
        public string validateEmail(string email) // interface method to validate email
        {
            var expr = @"^abc((\.[A-Z]+[a-z]*[0-9]*)|(\.[A-Z]*[a-z]+[0-9]*)|(\.[A-Z]*[a-z]*[0-9]+)?)?@bl\.co(\.[a-z]{2,})?$";
            return this.checkMatch(expr, email);
        }

        public string validateNumber(string number) // interface method to validate number
        {
            var expr = @"^([0-9]{2}[ ]{1}[0-9]{10})$";
            return this.checkMatch(expr, number);
        }

        public string validatePassword(string password) // interface method to validate password
        {
            var expr = @"^(?=.*[A-Z])(?=.*[0-9])(?=[^@#$%^&+=]*[@#$%^&+=][^@#$%^&+=]*$)(.{8,})$";
